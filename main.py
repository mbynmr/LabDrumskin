...
def main():
    ...
    match method.split("_")[0]:
        case "S":
            measure_sweep(freq=[400, 900], freqstep=5, t=0.5, devchan=dev_signal)
            output_file = resave_output(method="S", save_path=save_path)
            fit("outputs/output.txt", [0, 1])
        case "A":
            measure_adaptive(dev_signal, tolerance=5, start_guess=800, deltainit=1e2, bounds=[100, 1.2e3])
            output_file = resave_output(method="A", save_path=save_path)
            fit("outputs/output.txt", [0, 1], copy=False)
        case "P":
            measure_pulse_decay(dev_signal, runs=33)
            output_file = resave_output(method="P", save_path=save_path)
            fit("outputs/output.txt", [0.1, 0.6])
        case "AutoTemp":
            at = AutoTemp(save_folder_path=save_path, dev_signal=dev_signal, dev_temp=dev_temp)
            match method.split("_")[-1]:
                case "S":
                    at.auto_temp_sweep(temp_step=2.5, temp_repeats=5, freq=[50, 3600], freqstep=5)
                case "A":
                    at.auto_temp_adaptive(temp_step=2.5, temp_repeats=3,  # start_guess=2e3, tolerance=5,
                                          tolerance=5, start_guess=700, deltainit=1e2, bounds=[100, 9e2])
                case "P":
                    at.auto_temp_pulse(temp_step=2.5, temp_repeats=3, cutoff=[0.1, 0.2])
                case _:
                    at.sample_name = "_"
                    # at.calibrate()
            at.close()
